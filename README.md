# 🏛️ Maryland legislation file tree

Formatted, versioned legislative data for Maryland. Part of the [govbot](https://github.com/chihacknight/govbot) civic data project.
...
This is the **formatted data repo** for Maryland — one of two repos per jurisdiction:

1. **Scraper repo** (`govbot-openstates-scrapers/md-legislation`) — runs [OpenStates](https://github.com/openstates/openstates-scrapers) scrapers, produces raw scraped JSON
2. **This repo** (`md-legislation`) — reads the scraper repo's output and:
...
To change Maryland's configuration (branch pinning, runner, enabled jobs, labels), edit its entry in [`chn-openstates-files.yml`](https://github.com/chihacknight/govbot/blob/main/actions/pipeline-manager/chn-openstates-files.yml) and re-run `apply.py` — don't hand-edit the workflow files in this repo directly, since the next `apply.py` run will overwrite them.

Current settings for Maryland:

- **State code**: `md`
- **Toolkit branch**: `main`
- **Scraper repo**: `govbot-openstates-scrapers/md-legislation`
...
Pulls the latest scraped data from the scraper repo, sanitizes it, formats it into `country:us/state:md/sessions/...`, links events, and updates orphan tracking.
...
├── country:us/
│   └── state:md/            # state:usa for federal, state:il for Illinois, etc.
│       └── sessions/
│           └── {session_id}/
│               ├── bills/
│               │   └── {bill_id}/
...
Formatted metadata is saved to `country:us/state:md/sessions/`, organized by session and bill.
...
```json
{
  "HB999": {
    ...
    "vote_count": 2,
    "event_count": 0,
    "path": "country:us/state:md/sessions/103/bills/HB999"
  }
}
```
...
- Check the scraper repo (`govbot-openstates-scrapers/md-legislation`) actually got fresh data first — this repo only formats what that repo has already scraped
